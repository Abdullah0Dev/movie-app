...
//                         <Pressable className='flex flex-row ml-5 mt-4'>
...
import { View, Text, TouchableOpacity, Image, Pressable, ScrollView, FlatList } from 'react-native'
import React, { useState, useEffect } from 'react'
import { FontAwesome } from '@expo/vector-icons';
import { useLocalSearchParams } from 'expo-router'
import { LinearGradient } from 'expo-linear-gradient';
...
const PersonDetails = () => {
    ...
    return (
        <ScrollView className='bg-black h-full'>
            ...
            <View className='flex flex-row gap-3 ml-5 mt-[2vh] pt-2'>
                <Text className='text-medium text-base text-green-400'> <Text className='text-bold text-green-200  text-lg'> deathday </Text> {person.deathday || 'Alive'}</Text>
                <Text className='text-medium text-base text-white/50'> <Text className='text-bold text-white  text-lg'> birthday </Text> {person.birthday}</Text>
            </View>
            <View className='flex flex-row gap-3 ml-5 mt-1'>
                ...
                />
                <Text className='text-medium text-lg text-white'>{person.place_of_birth}</Text>
            </View>
            ...
            <View className='mt-4 mb-9'>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        <Text className='  ml-3 text-bold text-lg text-white/90 '>
                            {item}  {" "}
                        </Text>
                    )}
                    keyExtractor={({ item }) => item?.id}
